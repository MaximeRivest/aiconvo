// Chattering bridge extension for pi.
//
// Loaded by Chattering's per-operation RPC worker:
//   pi --mode rpc --no-extensions -e chattering-bridge.ts --session <file>
//
// It exposes pi's own runtime operations that plain RPC commands do not reach.
// Today that is one command:
//
//   /chattering-fork-at <entryId>
...
//     (SessionManager.createBranchedSession), so Chattering copies no internals.
//
...
export default function (pi: any) {
  pi.registerCommand("chattering-fork-at", {
    description: "chattering: fork the session through an entry (position 'at')",
    handler: async (args: string, ctx: any) => {
      const entryId = String(args || "").trim();
      if (!entryId) throw new Error("chattering-fork-at: missing entry id");
      const result = await ctx.fork(entryId, { position: "at" });
      if (result.cancelled) throw new Error("chattering-fork-at: fork was cancelled");
    },
  });
}
