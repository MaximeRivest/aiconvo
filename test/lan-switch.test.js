...
async function boot(t, extraEnv) {
  ...
  let log = '';
  const child = spawn(process.execPath, ['server.js'], { cwd: root, env: { ...process.env, HOME: home, PORT: String(port), AICONVO_TLS_PORT: String(tlsPort), AICONVO_NO_WATCH: '0', AICONVO_NO_LEDGER: '0', AICONVO_CACHE_DIR: path.join(home, 'cache'), AICONVO_CHECKPOINT_DIR: path.join(home, 'checkpoints'), AICONVO_DELEGATION_ROOT: path.join(home, 'delegations'), PI_CODING_AGENT_DIR: agent, PI_AGENT_DIR: agent, AICONVO_HOST: '', AICONVO_LAN: '', AICONVO_PUBLIC_URL: '', AICONVO_TOKEN: '', ...extraEnv }, stdio: ['ignore', 'pipe', 'pipe'] });
