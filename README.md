...
- **Models, branches, and headless runs (pi).** The blessed fast path next to the sovereign terminal:
  - **Model per conversation** — the `◇ model` chip in the conversation header sets the model through pi's own runtime (`set_model` over RPC). pi writes its native `model_change` tree entry, so resumes, branches, and forks inherit it. Assistant messages and tree turns show their model.
  ...
  - **Fan-out is web-only** — one terminal runs one model. With two or more model chips set, `ctrl+shift+enter` and `open & send` refuse with a hint; `ctrl+enter` fans out headlessly. Remove the extra chips to send in the terminal.
...
- **Windows.** The `⧉` button next to the brand opens a full-screen overview of every open view, with a miniature of each. Click a card to go there. **+ new window** starts a fresh home view and keeps the previous place. Each window keeps its route and any unsent composer text. Key **w**.
- The window list is **shared across devices**: desktop, phone, and e-ink all see the same windows (`~/.cache/aiconvo/windows.json`, live over SSE). Each device keeps its own *active* window, scroll positions, and attached images. Closing a window removes it everywhere; a window that another device is using comes back when that device acts again. Unsent text follows the window with last-writer-wins.
