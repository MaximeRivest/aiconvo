...
- **File workspace:** **Live** edits the current file in MRMD (Markdown autosaves and runnable code cells) or CodeMirror (code uses explicit Ctrl+S). **History** opens a read-only chronological drawer: read one saved version, step older/newer, or compare two versions. The former keyframe strip is hidden, and historical files load only when requested. Navigation returns to the folder browser or originating conversation. Conversation drafts and reading positions are retained; the open file is remembered for the conversation during this page session.
