...
- **Side panel layout** (settings → appearance → layout, saved per browser): **Home, Inbox, Agents, Recent** lead the rail. Below a separator, the project picker and **Projects, Chats, Files** support project browsing. Machine switching and Settings sit at the bottom. Panel choices leave the main page intact; Home opens the timeline. Projects offers search and sorting. Expanded Gantt charts stay beside—not beneath—the sidebar.
- **Long sidebar lists** load more as you scroll, with a Show more fallback. The project picker chooses **All projects**, a named project, or **No project** for Chats and Files only. **Recent** shows file activity across all projects. Files and Recent share the **human | agents | both** choice; existing retention bounds still apply. See [recent file activity](design/47-recent-file-activity.md).
- **Inbox** holds machine-wide unread replies and read history; interrupted agents are visible without opening a fold. **Agents** shows work across all projects, with idle processes tucked away. Opening a reply never hides other projects' activity. **Return to work** goes back to the page left for an agent check-in, even after several check-ins or a refresh. Back/Forward continues to restore project scope. See [work-first navigation](design/51-work-first-navigation.md).
- **Rail counts** on Agents and Inbox are off by default (quiet dots instead). Enable them in Appearance → **Show counts on Agents and Inbox icons**. Both are machine-wide; routine background jobs never light Inbox. `` ` `` folds the panel while leaving the rail usable; phones keep the top bar. Themes shape the panel with `--panel-bg`, `--panel-r`, `--panel-row-r`, `--panel-border`. The conversation title stays quiet and the composer stays pinned.
