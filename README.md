...
- **Side panel layout** (settings → appearance → layout, saved per browser): the same list becomes a column on the left that stays open: home, **+ new** (in this project when you are in one; `▾` for the other kind), pinned, unread and read replies, **recent** (conversations you last wrote in, and file activity) with one `project | all` toggle. Files additionally offer **human | agents | both**: human editor opens/saves or confirmed agent reads/writes/edits, deduplicated and newest first. Both choices are remembered; see [recent file activity](design/47-recent-file-activity.md). At the bottom is a dock: folded sections, traffic (running agents and processes) and notifications. Fold a section and it moves to the dock; the fold is remembered. Themes shape the panel with `--panel-bg`, `--panel-r`, `--panel-row-r`, `--panel-border` (see `design/tokens.css`). The top bar goes away entirely: in a conversation a small transparent title line sits above the page (it hides while you scroll down and returns when you scroll up), the composer stays pinned to the bottom, and the project area opens from the button next to home. `` ` `` folds the column to a rail. Phones keep the top bar. See [design/42](design/42-side-panel-and-inbox-controls.md).
