...
Every install keeps a roster of the people admitted on it (settings → people). The owner is the person whose account the machine is; they sign in with the install token, so existing devices keep working. Other people get an invite link, shown once. The header shows who you are and who else is on this machine right now; a conversation says who is typing in it, and every message, file save and vouch is recorded by name (the home filter has a "mine" choice). When you switch machines from the header, you arrive there as yourself: paired installs exchange a signing key and hand you over with a short signed note, no paste.
...
- **Side panel layout** (settings → appearance → layout, saved per browser): a machine/workspace selector and a rail for **Projects, Chats, Files, Agents, Inbox**. Each chooses the neighboring panel without replacing the open page. Projects offers search and sorting by recent activity, name, conversation count, folder date or disk size; click a row to open the project overview. Home still opens the whole-system timeline. Expanded Gantt charts stay inside the content area, beside—not beneath—the sidebar.
- **Long sidebar lists** load more as you scroll, with a Show more button as a fallback: no silent eight-item cutoff. The dropdown beside Home chooses **All projects**, a named project, or **No project** for Chats, Files and Agents together. Files also offers **human | agents | both**, remembered independently. The file store's existing retention bounds still apply. See [recent file activity](design/47-recent-file-activity.md) and [the Projects directory](design/49-project-directory-and-sidebar-lists.md).
- **Inbox** holds machine-wide unread replies, read history and background jobs in separate tabs. Chats contains only pinned/recent conversations within the selected scope. Opening an Inbox reply selects its project; switching scope never reassigns a conversation. Back/Forward restores scope along with the page. See [workspace scope and Inbox](design/50-workspace-scope-and-inbox.md).
- **Rail counts** on Agents and Inbox are off by default (quiet dots instead). Enable them in Appearance → **Show counts on Agents and Inbox icons**. Agents counts follow project scope; unread indicators belong to the machine-wide Inbox, not Chats. `` ` `` folds the panel while leaving the rail usable; phones keep the top bar. Themes shape the panel with `--panel-bg`, `--panel-r`, `--panel-row-r`, `--panel-border`. The conversation title stays a quiet transparent line, the composer stays pinned to the bottom, and project scope is selected from the dropdown next to Home.
