...
- `server.js:6939` `alacrittyBin()` probes snap paths; terminal launch, Claude continuation and `chattering-bridge.py` assume Alacritty + X11 (`DISPLAY`, `XAUTHORITY`, `server.js:7014-7022`).
...
**Job to be done.** Several people — a household now, a small team or a class next — work in one Chattering with clear identities, attribution, permissions and budgets; an owner can see and administer all of it; a new machine can be brought to the same version reliably.
