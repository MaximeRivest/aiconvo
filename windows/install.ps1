...
foreach ($location in $locations) {
    $path = Join-Path $location 'chattering.lnk'
    ...
    $shortcut.Description = 'Start Chattering and open its window'
    $shortcut.WindowStyle = 7
    $shortcut.Save()
}
Write-Output "Installed Chattering launcher for $Distro / $LinuxUser on port $Port."
