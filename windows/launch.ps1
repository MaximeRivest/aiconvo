...
function Test-ChatteringReady([string]$Url) {
    $response = $null
    try {
...
try {
    ...
    $command = 'systemctl --user start chattering.service && exec flock -n "$XDG_RUNTIME_DIR/chattering-launcher.lock" sleep infinity'
    ...
    while ((Get-Date) -lt $deadline) {
        if (Test-ChatteringReady "http://127.0.0.1:$($config.Port)/health") {
            $ready = $true
            break
        }
        Start-Sleep -Milliseconds 500
    }
    if (-not $ready) { throw "Chattering did not start within 90 seconds. Open your WSL terminal and run: systemctl --user status chattering" }
    if ($CheckOnly) { Write-Output "Chattering is ready at $url"; return }
    ...
    $tokenRaw = & "$env:SystemRoot\System32\wsl.exe" -d $config.Distro -u $config.User --exec cat "/home/$($config.User)/.cache/chattering/lan-token" 2>$null
    ...
    [System.Windows.Forms.MessageBox]::Show($_.Exception.Message, 'Chattering could not open', 'OK', 'Error') | Out-Null
    exit 1
}
