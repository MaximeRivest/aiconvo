...
$function = $ast.Find({ param($node)
    $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -eq 'Test-ChatteringReady'
}, $true)
...
try {
    ...
    foreach ($status in @(200, 401, 403, 500)) {
        $actual = Test-ChatteringReady "http://127.0.0.1:$port/"
    ...
    if (Test-ChatteringReady "http://127.0.0.1:$port/") { throw 'A closed port must not be ready' }
