# Chattering semantic stage (GPU server)

Late-interaction (ColBERT) search that runs on the GPU server and adds
meaning-based matches to chattering search. The laptop stays authoritative:
...
```bash
ssh lambda-quad
mkdir -p ~/family-ai/chattering-semantic && cd ~/family-ai/chattering-semantic
uv venv --python 3.12 .venv
uv pip install --python .venv/bin/python pylate fastapi 'uvicorn[standard]'
# copy semantic_server.py here, then:
sudo cp chattering-semantic.service /etc/systemd/system/
sudo systemctl daemon-reload && sudo systemctl enable --now chattering-semantic
...
- Enable in Chattering settings → semantic search (off by default).
...
- The host pushes changed units (titles, user and assistant messages,
  ...
  (`semsync` table in `~/.cache/chattering/search.db`).
